...
function Tabel() {
    ...
    const columns = useMemo(
        () => [
            ...
            },
            {
                Header: "Aksi",
                Cell: (props) => {
                    const edit_onClick = async () => {
                        const data = props.cell.row.original;
                        dispatch(activated_modal_changed({ currentActiveModal: "EditRuas", data: data }));
                        dispatch(modal_changeState());
                    };
                    const lihat_onClick = () => {
                        const data = props.cell.row.original;
                        dispatch(activated_modal_changed({ currentActiveModal: "DetailRuas", data: data }));
                        dispatch(modal_changeState());
                    };
                    const hapus_onClick = () => {
                        const data = props.cell.row.original;
                        dispatch(activated_modal_changed({ currentActiveModal: "DeleteRuas", data: data }));
                        dispatch(modal_changeState());
                    };
                    return (
                        <div className="flex items-center justify-center gap-2">
                            <div onClick={edit_onClick}>Edit</div>
                            <div onClick={lihat_onClick}>Lihat</div>
                            <div onClick={hapus_onClick}>Hapus</div>
                        </div>
                    );
                },
            },
    ...
    const GlobalFilterComp = useCallback(() => {
        return <GlobalFilter filter={globalFilter} setFilter={setGlobalFilter} />;
    }, []);
